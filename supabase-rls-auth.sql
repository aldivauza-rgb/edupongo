-- =============================================================
-- RLS POLICIES — AUTHENTICATED-ONLY WRITE
-- Jalankan di Supabase SQL Editor setelah mengaktifkan Auth
-- =============================================================
...
-- 5. Policy untuk Partner (edp_partners) — kalau tabelnya sudah ada
-- drop policy if exists "anon_can_insert_edp_partners" on edp_partners;
-- drop policy if exists "anon_can_update_edp_partners" on edp_partners;
-- drop policy if exists "anon_can_delete_edp_partners" on edp_partners;
-- create policy "auth_insert_edp_partners" on edp_partners for insert to authenticated with check (true);
-- create policy "auth_update_edp_partners" on edp_partners for update to authenticated using (true);
-- create policy "auth_delete_edp_partners" on edp_partners for delete to authenticated using (true);
