...
/* ─── Ikon SVG ───────────────────────────────────────────── */
function Icon({ d, size = 19, stroke = "currentColor", strokeWidth = 1.5 }) {
  return (
...
/* ─── Sidebar Button ─────────────────────────────────────── */
function SidebarBtn({ icon, label, active, disabled, onClick, badge, chevron, chevronOpen, style }) {
  const bg = active ? "#046CF2" : "transparent";
  const color = active ? "#F9F9F9" : "#354764";
  return (
    <button
      onClick={disabled ? undefined : onClick}
      disabled={disabled}
      style={{
        width: "100%", height: 48, borderRadius: 12, background: bg,
        border: "none", padding: "0 12px", display: "flex",
        alignItems: "center", gap: 10, cursor: disabled ? "not-allowed" : "pointer",
        transition: "background 0.15s", opacity: disabled ? 0.5 : 1,
        fontFamily: "Inter", fontSize: 14, fontWeight: 500,
        textAlign: "left", color, lineHeight: 1.4, ...style,
      }}
      onMouseEnter={e => { if (!active && !disabled) { e.currentTarget.style.background = "#051B3E"; e.currentTarget.style.color = "#F9F9F9"; }}}
      onMouseLeave={e => { if (!active && !disabled) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#354764"; }}}
    >
      <Icon d={icon} stroke={active ? "#F9F9F9" : "#354764"} />
      <span style={{ flex: 1, textAlign: "left" }}>{label}</span>
      {badge != null && (
        <span style={{
          minWidth: 24, height: 24, borderRadius: 12, padding: "0 8px",
          background: "#E6F0FE", color: "#046CF2",
          fontFamily: "Inter", fontWeight: 600, fontSize: 12,
          display: "inline-flex", alignItems: "center", justifyContent: "center",
        }}>{badge}</span>
      )}
      {chevron && (
        <Icon d={chevron} stroke="#354764" size={16}
          style={{ transform: chevronOpen ? undefined : "rotate(-90deg)", transition: "transform 0.2s" }}
        />
      )}
    </button>
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "none", padding: "0 12px 0 41px", display: "flex",
        alignItems: "center", gap: 10, cursor: "pointer",
        transition: "background 0.15s",
        fontFamily: "Inter", fontSize: 13, fontWeight: 500,
        textAlign: "left", color: active ? "#F9F9F9" : "#97A2B0", lineHeight: 1.4,
      }}
      onMouseEnter={e => { if (!active) { e.currentTarget.style.background = "#051B3E"; e.currentTarget.style.color = "#F9F9F9"; }}}
      onMouseLeave={e => { if (!active) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#97A2B0"; }}}
    >
      <span style={{ width: 5, height: 5, borderRadius: "50%", background: active ? "#F9F9F9" : "#97A2B0", flexShrink: 0 }} />
      {label}
    </button>
  );
}
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "Inter, system-ui, sans-serif", background: "#E8E9F1" }}>
      ...
      <aside style={{
        ...
        background: "#010E23", padding: "28px 16px",
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
          <SidebarBtn icon={ICONS.chart} label="Analisis Web" disabled />
          <SidebarBtn icon={ICONS.monitor} label="Slideshow" disabled />

          {/* Profil — collapsible */}
          <SidebarBtn icon={ICONS.user} label="Profil"
            onClick={() => setProfilOpen(o => !o)}
            chevron={ICONS.chevronR} chevronOpen={profilOpen}
          />
          {profilOpen && (
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {["Sambutan", "Tentang Kami", "Visi & Misi", "Struktur Organisasi", "Akreditasi", "Statistik"].map(s => (
                <SubItem key={s} label={s} active={false} />
              ))}
            </div>
          )}

          {/* Informasi — collapsible, default open */}
          <SidebarBtn icon={ICONS.bell} label="Informasi"
            onClick={() => setInfoOpen(o => !o)}
            chevron={ICONS.chevronD} chevronOpen={infoOpen}
          />
          {infoOpen && (
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <SidebarBtn icon={ICONS.gear} label="Kelola Akun" disabled />

          {/* Separator */}
          <div style={{ margin: "16px 8px 8px", display: "flex", flexDirection: "column", gap: 3 }}>
            <SidebarBtn icon={ICONS.inbox} label="Pemberitahuan" badge="2" />
          </div>
        </nav>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {/* User block */}
          <div style={{
            background: "#051B3E", borderRadius: 12, padding: 12,
            display: "flex", alignItems: "center", gap: 12, cursor: "pointer",
          }}>
            <div style={{
              width: 32, height: 32, borderRadius: "50%",
              background: "#046CF2", display: "flex", alignItems: "center",
              justifyContent: "center",
              fontFamily: "Inter", fontWeight: 600, fontSize: 13, color: "#F9F9F9",
              flexShrink: 0,
            }}>A</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: "Inter", fontWeight: 500, fontSize: 13, lineHeight: 1.4, color: "#F9F9F9" }}>admin</div>
              <div style={{ fontFamily: "Inter", fontSize: 12, lineHeight: 1.5, color: "#5D6B82" }}>Administrator</div>
            </div>
            <Icon d={ICONS.chevronR} size={18} stroke="#354764" />
          </div>
          ...
          <button onClick={onLogout} style={{
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "#051B3E"; e.currentTarget.style.color = "#ef4444"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "rgba(255,255,255,0.6)"; }}
          >
            <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
              <path d="M9.17 14.83l5.66-5.66M14.83 14.83L9.17 9.17"/><path d="M12 22c5.5 0 10-4.5 10-10S17.5 2 12 2 2 6.5 2 12s4.5 10 10 10z"/>
            </svg>
            Keluar
          </button>
        </div>
      ...
      {/* ─── MAIN ───────────────────────────────────────────── */}
      <main style={{ flex: 1, overflowY: "auto", minWidth: 0, background: "#E8E9F1" }}>
        <div style={{ padding: "28px 32px 0" }}>
          <h2 style={{ fontSize: "clamp(18px, 1.4vw, 24px)", fontWeight: 700, color: "#010E23", margin: 0, fontFamily: "Inter, sans-serif" }}>
            {activeLabel}
          </h2>
        </div>
        <div style={{ padding: "20px 32px 32px", maxWidth: 1400 }}>
          {PAGES[page]?.render()}
        </div>
      </main>
    </div>
