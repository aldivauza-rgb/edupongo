...
export default function ProblemPage() {
  ...
  const load = async () => { try { setItems(await problemApi.getAll() || []); } catch (e) {} setLoading(false); };
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    try {
      if (edit) await problemApi.update(edit, form); else await problemApi.create(form);
  ...
  const remove = async (id) => {
    ...
    try { await problemApi.remove(id); setMsg('Terhapus!'); load(); } catch (e) { setMsg('Gagal hapus'); }
  };
      ...
      {loading ? <div className="admin-loading">Loading...</div> : (
        <div className="admin-table-wrap">
          <table className="admin-table">
            <thead><tr><th>Icon</th><th>Judul</th><th>Urutan</th><th>Status</th><th>Aksi</th></tr></thead>
            <tbody>
              {items.map(item => (
                <tr key={item.id}>
