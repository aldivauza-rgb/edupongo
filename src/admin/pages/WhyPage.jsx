...
export default function WhyPage() {
  ...
  const load = async () => { try { setItems(await whyApi.getAll() || []); } catch (e) {} setLoading(false); };
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    try {
      if (edit) await whyApi.update(edit, form); else await whyApi.create(form);
  ...
  const remove = async (id) => {
    ...
    try { await whyApi.remove(id); setMsg('Terhapus!'); load(); } catch (e) { setMsg('Gagal hapus'); }
  };

  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <h2>Kenapa Edupongo</h2>
        <button className="admin-btn admin-btn-primary" onClick={openNew}>+ Tambah</button>
      </div>
        ...
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id}>
                  <td className="admin-cell-content"><span className="admin-text-preview">{item.title}</span></td>
              ...
              {items.length === 0 && <tr><td colSpan={4} className="admin-empty">Belum ada data</td></tr>}
            </tbody>
          </table>
        </div>
      )}
